...
- [x] Keyboard layout finalized (24 keys, C3–B4) — geometry source of truth: [`hardware/3d/keyboard/octaves-plan.md`](../hardware/3d/keyboard/octaves-plan.md)
...
- [ ] **Milestone: v1.0 — one-octave device.** First complete integration after the first 3D prints: a working one-octave groovebox — one octave C–B (**12 keys**), **1× PCF8575, hand-wired**, printed caps + plate, **enclosure with ST7789 display + 40mm speaker + 2× EC11 encoders + joystick**, perfboard main board — **no LEDs, no custom PCB** (the SK6812 LEDs require a PCB that won't be made in this version; deferred). The 2-octave 24-key keybed is the target for a later version. Layout: [`docs/hardware/enclosure-layout.svg`](hardware/enclosure-layout.svg). Background: [0005](explorations/0005-proto-keyboard-module.md). ("v1.0" ≠ the V1/V2 *design* iterations used elsewhere in the docs.)
  ...
  - [x] **Polish `plate.scad`** ✅ (2026-08-02): fit-test extras integrated as **parametric ghosts** (KS-33 built from doc dims — no Gateron mesh — with `key_rot()` D#/F# rotation, right-angle header, PCF board + opt-in module-mesh import), **feet** (7× ø6×7.2, `feet` flag — plate stands flat with switches + PCF + header mounted, 0.8 air over the header tails), **tab at full `prad 3.2`** + concave `tab_fillet 2.4` at the junction (plate body = ONE closed outline, chamfered via minkowski — the hull trick only did convex), **wire channels re-routed/re-enabled** against the vertical module (asserts vs all bosses), **elephant-foot relief on the cutout top rims** (`hole_chamfer 0.15` — a 45° chamfer, 0.15 wide × 0.15 tall, hard-capped laterally by the Silent's 14.5 flange seat on the 14.0 hole; *the 0.4-tall steeper-than-45° variant was documented but never built — corrected 2026-08-03, the in-print STL carries the 45°×0.15 relief*), **root coves + tip chamfers** on bosses (0.4/0.4) and feet (0.8/0.4), module nudged north to **cy 24.4** so every cove lands fully on solid web
  - [x] **Wire channels ROUTED for real (2026-08-03)** — the generic trunk+spurs got replaced by the actual electrical routing, designed in [`hardware/3d/keyboard/wiring-plan.svg`](../hardware/3d/keyboard/wiring-plan.svg) and translated 1:1 into `plate.scad` as per-octave route data (11 wires · 26 segments, generic asserts: ≥0.8 web between routes, boss/foot keep-outs): **12 signals** (5 web lanes y 9.2–18.8 + pin-side spurs → open mouths at the board edge; D#/F#/E/F fly through open air) with hole assignment E→P07 D→P06 C→P05 C#→P04 D#→P01 / F→P10 G→P11 A→P12 B→P13 A#→P14 G#→P15 F#→P16 (P00/P02/P03/P17 spare, geometry rules — firmware remaps); **GND = two chains** (row-GND north holes → D#/F# → sharps passages y 24 → side drop columns C−0.1 / B+4.6 → **ONE independent straight gutter** in the front apron, `gnd_gap` 2.0 walled off the pockets — the wire sags under the wall to visit each GND pin); **rounded mouths** ø2 wherever a channel opens into a pocket. The map is drawn in the **soldering view** (from below, C right, P0x row right; solid = channel, dashed = wire in air). Module row orientation (P0x west when mounted) derived from the module photo — confirm silkscreen at assembly. **Routing + pin map superseded the same day by the polish below**; `wiring-plan-base.svg` deleted
  - [x] **Wiring SVG POLISHED with the user — FROZEN (2026-08-03)**: `wiring-plan.svg` is now THE routing source of truth. Web lanes moved to the **exact PCF pin heights** (Δ2.54 · model y 18.01 / 15.47 / 12.93 / 10.39) so every channel enters its pin dead straight, both sides. **Final pin map: C→P04 · D→P05 · E→P06 · F→P07 · C#→P02 · D#→P01 / A→P10 · B→P11 · A#→P12 · G#→P13 · F#→P15 · G→P17** (free: P00 · P03 · P14 · P16 — firmware remaps anyway). **A#/G# switch pins inverted** (signal = left pin, GND = right pin in this view) so the left GND chain runs flat at pin height: GND → F# → S-drop centered in the F#–G# web → G# → A# (GND pins visited in-line) → side drop x 61 → B → gutter. Right GND chain: GND → D# → high passage at D#-pin height over the C# pocket → C# → Z-descent inside the C#–C web (flat middle centered between the holes) → straight down through C's pin → gutter. Front gutter centered, ends 1mm inside the outer pocket corners (x ±63). Two **under-board routes**: G→P17 in a channel squeezed **between the south bosses** (2.1 gap vs 1.6 channel, 0.25 walls) and F→P07 crossing the E pocket at the pillar limit then climbing under the board to its pin. C#→P02 runs flat at its own pin height through the D#–C# web
  - [x] **SVG re-translated into `plate.scad` + design iteration on top (2026-08-03, user-reviewed live)**: routes are now polylines with **swept 90° bends** (`chan_bend 1.2`), lanes at the exact pin heights **derived** from the module doc (`pcf_pin_edge 7.2` + `pcf_pin_row_inset 1.86`), final pin map ported. On-review deltas vs the frozen SVG: **naturals GND now runs in channels** — the gutter's ends curl 90° into the C/B GND pins and **double-width spur tees** (2.8, two cables in+out, own 90° sweeps) reach D–A's pins; **F→P07 got its under-board climb** (x = P0x row, straight into the pin); G's F-crossing lowered to **y 5.2** (was 0.9 from the pocket edge); channel slimmed to **1.4 × 1.2** (wire sits 0.2 proud) with **`chan_rim 0.3` mouth break + rounded (half-revolved) end caps**, ø2 pocket mouths deleted; **`hole_rim 0.3`** chamfer on the pocket rims of the channel face (chamfer > fillet: prints identical at this scale, consistent 45° language); **C#|D# back foot dropped** (the GND high passage owns that web) and the back foot moved **G#|A# → F#|G#** (the S-drop's top leg ducks 1.2 below F#'s pin height to clear it); header tails now **trimmed after soldering** (`hdr_tail` 2.0→1.0) → **feet 7.2→6.0**; **M2 pilots up every foot** (ø1.8×5.2 — M2×6 on a 2.4 deck / M2×8 on a 3.2–4.0 deck) — the feet double as **deck-mount bosses** for the pre-enclosure integration deck (modules screw onto a common floor; a future `deck.scad` derives the pattern from `plate_feet()`/`plate_bbox()` accessors). `plate.stl` re-exported. **`wiring-plan.svg` now TRAILS `plate.scad`** — the scad route data is the routing source of truth; SVG re-sync pending
  - [x] **`hardware/3d/` restructured — one folder per module family (2026-08-03)**: `3d/keyboard/` (scad sources + `octaves-plan.md` + `wiring-plan.svg` + `stl/` exports) + `3d/vendor/` (third-party meshes — **decision changed: KEPT and committed for reuse**, provenance + caveats in `vendor/README.md`; visual ghosts only, never dimensional truth); future modules get sibling folders (`deck/`, `enclosure/`). All repo references updated (docs, CLAUDE.md, scad imports, cad skill)
  - [x] **Model code cleanup + ghosts → `assembly.scad` (2026-08-03)**: ghost decision = **both, mesh opt-in** — ghost modules stay DEFINED in `plate.scad` (they read its params; `use <>` imports modules, not variables) but the show_* flags + composition moved to `assembly.scad`: parametric ghosts (switches/PCF/header — fast, Silent-correct) as the default, **vendor meshes opt-in** (`show_pcf_mesh` — true component detail; `show_switch_mesh` — ⚠️ standard KS-33, visual only); `plate.scad` renders only the printable plate (+ sections), redundant `cap_ghosts` deleted (assembly places the real caps), assembly derives its layout via the new `plate_octaves()` accessor; `plate-pcf-fit.scad` DELETED (role absorbed by assembly + vendor/)
  - [x] **Settled params FROZEN (2026-08-03)**: Customizer now shows only the real knobs — `plate.scad`: `octaves` · `kerf` · `feet` (the `section`/`section_bay` cutaways were later dropped the same day as not useful); `key_cap.scad`: `part` · `split_post` · `section`; `assembly.scad`: view flags + `pressed_key` + colors. Everything print-validated/decided moved under `/* [Hidden] */` as named grouped constants (dates on each group). **Geometry verified unchanged**: exports compared as canonical vertex sets against the in-print `plate.stl` — the delta (11/13 of ~47.7k vertices) is below OpenSCAD's own run-to-run tessellation noise (16/17); cap sources differ only in comments vs HEAD
  - [x] **`wiring-plan.svg` re-synced to the as-built routing (2026-08-03)** — cleanup session COMPLETE: gutter redrawn with its 90° end-curls into the C/B GND pins + the five double-width spur tees at D–A (gutter y −10.3 at the 1.4 slot), F→P07's under-board climb channel at the P0x column, G's F-crossing at y 5.2 and its channel ending at the F# pocket edge (last stretch to the pin = air), S-drop top leg at the 1.2 duck, notes updated (channel 1.4 × 1.2 + rim 0.3, two under-board climb channels). The SVG header now states it REFLECTS `plate.scad` — the scad route data remains the routing source of truth
  - [ ] Print **batch 2**: plate-module + 12 caps → solder the octave → a ready-to-use/integrate keyboard module *(plate.stl sent to print 2026-08-03)*
  - [ ] **Integration deck (pre-enclosure, idea logged 2026-08-03)** — a printed base floor all modules screw onto (keyboard now, joystick + screen/speaker modules later) so everything plays together **before** the final compact shell exists. `deck.scad` derives each module's screw pattern from its accessors (keyboard: `plate_feet()` / `plate_bbox()`, M2 self-tap into the foot pilots — deck 2.4 → M2×6, deck 3.2–4.0 → M2×8; **no inserts at deck level**, the insert + countersunk-M2 hardware goes to the batch-3 shell). **PCF support**: the deck carries a block or two bosses under the module aligned with its **south mounting-hole pair** (`plate_pcf_bosses()`), either as a rest for the board or through-screwed (longer M2 up through deck boss + board hole into the plate boss pilot) — keep clear of the solder-face wire joints (~2 mm) and the trimmed header tails; the solder face sits **1.6 above the deck floor** (`plate_foot_h() + plate_pcf_z()`)
  ...
  - [ ] Hand-wire the octave: switch pins → PCF8575 module via plate wire channels — **no custom PCB** (decided 2026-07-31, see [`octaves-plan.md`](../hardware/3d/keyboard/octaves-plan.md)); PCF mounts into the plate between the sharps. SK6812 LEDs: **deferred** — they need a PCB, which is left to a future PCB version
