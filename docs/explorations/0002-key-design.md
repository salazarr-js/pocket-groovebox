...
- ✅ Toolchain: layout + plate + caps in **OpenSCAD** — see [0004-cad-toolchain.md](0004-cad-toolchain.md). *(KiCad since dropped for V1 — no custom PCB, keyboard hand-wired.)*
- 🚧 First build: **build 1.0** — a one-octave module (12 keys, C–B, hand-wired, no LEDs) before the full 24 — see [0005-proto-keyboard-module.md](0005-proto-keyboard-module.md).
...
**Decided: Gateron KS-33 Low Profile 2.0 Silent Brown.** Key values: silent tactile, **55 ± 15 gf** operating force, **15 × 15 mm** body with MX-cross stem. All other dimensions (travel, stem cotas, plate cutout/thickness, the dimensional drawing) live in the [module doc](../hardware/modules/gateron-ks-33.md) — the single source for switch specs.
...
**Range: C3–B4 = 24 keys** (14 naturals + 10 sharps, no top C5). Two rows of flat rectangular caps: naturals in the front row, sharps offset in a back row (with the usual gaps at E–F and B–C), like the OP-1.

```
  ▢ ▢     ▢ ▢ ▢     ▢ ▢     ▢ ▢ ▢     ← sharps (back row, 10)
▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢ ▢          ← naturals (front row, 14)
...
- **No PCB — hand-wired**: 24 switches wired directly to 2× PCF8575 (no diodes, common GND — see [module doc](../hardware/modules/gateron-ks-33.md#wiring-to-the-keyboard-i2c-expanders)). The plate carries the switches; wiring routes to the expander modules.
- **3D-print files needed**: (1) natural cap, (2) sharp cap, (3) plate with 14 mm cutouts +
  ...
  fit, and feel before committing to all 24.
