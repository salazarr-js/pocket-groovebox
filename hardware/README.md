...
**dropped for V1**: the keyboard is hand-wired, no custom PCB (see [`3d/keyboard/octaves-plan.md`](3d/keyboard/octaves-plan.md)).
*(Ergogen was evaluated and dropped; v0's `octave-layout.scad` was deleted — the layout lives inside `plate.scad` per [`3d/keyboard/octaves-plan.md`](3d/keyboard/octaves-plan.md).)*
...
**Status: clean rebuild** — the v0 sources were print-validated (PLA/PETG, 0.2 & 0.4 mm nozzles, 3 print services) and then deleted on purpose; [`3d/keyboard/octaves-plan.md`](3d/keyboard/octaves-plan.md) holds the full rebuild plan, calibrated values and source snapshots. Target: **`key_cap.scad` + `plate.scad` + `assembly.scad`**, then incremental integration toward the two-shell enclosure (top shell absorbs the plate + display/speaker/encoders/joystick; bottom shell carries a perfboard main board).
...
- This list is a summary — values live in [`3d/keyboard/octaves-plan.md`](3d/keyboard/octaves-plan.md) (single source).
