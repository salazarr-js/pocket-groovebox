...
## Structure (settled 2026-08-03 — one folder per module family)

```
3d/
├── keyboard/            ← the keybed module (build 1.0: one octave)
│   ├── octaves-plan.md  ← THE rebuild plan — calibrated values, source snapshots (single source of values)
│   ├── key_cap.scad · plate.scad · assembly.scad
│   ├── wiring-plan.svg  ← soldering-view routing map (trails plate.scad — scad route data is the routing source of truth)
│   └── stl/             ← our exports (plate.stl, key_cap-*.stl)
├── vendor/              ← third-party reference meshes, committed — provenance + caveats in vendor/README.md
└── (future: deck/, enclosure/)
```

**The v0 sources were deleted on purpose (2026-07-31)** after the print validation — clean rebuild since. Everything needed is in [`keyboard/octaves-plan.md`](keyboard/octaves-plan.md) (plan, calibrated values, verbatim source snapshots) + the design notes below. File structure per plan §5: **`key_cap.scad` · `plate.scad` · `assembly.scad`** — three files, layout + cutout module live inside `plate.scad`.
...
### Plate (v0 `plate-octave.scad` / `switch-cutout.scad`, deleted — snapshot in [`octaves-plan.md`](keyboard/octaves-plan.md) §7)
...
- **Bottom-edge chamfers** against service-side elephant foot (see Print-validated) — ✅ **plate done (2026-08-02)**: outline chamfered on both faces (`edge_chamfer 0.3`) + top-rim chamfer on the switch cutouts (`hole_chamfer 0.15` — 45°, capped by the Silent's 14.5 flange seat on the 14.0 hole, so 0.3 is NOT possible there; 0 restores the exact print-validated geometry). Enclosure still pending.
...
- ✅ **Pitch** — resolved for now: **18.8 mm** (cap 18 + gap 0.8, parametric, rev. 2026-08-01 — [`octaves-plan.md`](keyboard/octaves-plan.md) §7); revisit after build 1.0.
- ✅ **Sharps** — positions follow the layout snapshot in [`octaves-plan.md`](keyboard/octaves-plan.md) §7 (C#–D# at 1.5·pitch, F#–G#–A# at 1.25·pitch).
