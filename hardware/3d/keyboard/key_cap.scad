...
// Spec: hardware/3d/keyboard/octaves-plan.md §7 (cap params — the single source of values);
...
// (untested print experiment — off = the print-validated geometry)
split_post = false;
...
// ---- everything below is FROZEN (official values, print-validated fits or
// closed design decisions — dates on each group). Not Customizer knobs. ----
/* [Hidden] */
$fn = $preview ? 32 : 96;
eps = 0.01;
...
// ---- directional profile (0.4 grid, closed 2026-08-01) ----
...
module cap_cavity(w, d, ins) {
  k = (cavity_top - base_rim_height) / (bevel_end - base_rim_height);
  union() {
...
// central stem post, foot chamfered on the outside (same 45° anti-elephant-
// foot). Named stem_post: plate.scad also exports a post() (bosses/feet) and
// assembly.scad `use <>`s both files into one namespace
module stem_post() {
